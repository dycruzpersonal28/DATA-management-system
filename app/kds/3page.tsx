...
type ReceiptItem = {
  ...
  note: string | null
  modifiers: any[]
  addons: Array<{ id?: string; name: string; price?: number; quantity: number; ingredients?: { name: string; quantity: number }[] }>
  category_id?: string
          ...
          <div key={`${item.id}-${idx}`} className="border-b border-gray-800 pb-2 last:border-0 last:pb-0">
            ...
            {item.modifiers?.length > 0 && (
              <div className="mt-0.5 space-y-0.5">
                {item.modifiers.map((m: any, i: number) => {
                  ...
                  if (m.type === 'discount_id') {
                    // Don't show the raw ID number on the kitchen ticket —
                    // just confirm the ID photo was captured for this discount.
                    return (
                      <p key={i} className="text-xs text-emerald-400">
                        📷 ID uploaded
                      </p>
            ...
            {item.addons?.length > 0 && (
              <div className="mt-0.5 space-y-0.5">
                {item.addons.map((a, i) => (
                  <div key={i}>
                    <p className="text-xs text-indigo-400">+ {a.name}{a.quantity > 1 ? ` ×${a.quantity}` : ''}</p>
...
export default function KdsDisplayPage() {
  ...
  const loadOrders = useCallback(async (force = false) => {
    ...
    const addonItemIds = [...new Set(
      (allItems ?? []).flatMap((i: any) => Array.isArray(i.addons) ? i.addons.map((a: any) => a.id).filter(Boolean) : [])
    )]
    const allLookupIds = [...new Set([...itemIds, ...addonItemIds])]
    ...
    for (const receipt of receipts) {
      ...
      if (!selectedStation.show_receipt) {
      ...
      built.push({
        ...
        items: items.map(i => ({
          ...
          addons: Array.isArray(i.addons) ? i.addons.map((a: any) => ({
            ...a,
            ingredients: ingredientsByItem.get(a.id) || [],
          })) : [],
          ingredients: ingredientsByItem.get(i.item_id) || [],
        })),
      })
    }

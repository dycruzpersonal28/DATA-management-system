...
export default function InventoryLogPage() {
  ...
  const groupedRows = useMemo<GroupedRow[]>(() => {
    const groups = new Map<string, UnifiedLog[]>()
    const singles: UnifiedLog[] = []

    for (const log of filtered) {
      if ((log.source === 'sale' || log.source === 'void') && log.receipt_number) {
        // Group by receipt + product, so a receipt with two different
        // finished products sold shows as two rows, each with its own
        // ingredient breakdown. Rows with no product_name (pre-migration
        // sales) fall back to one shared bucket per receipt, same as before.
        const productKey = log.product_id ?? log.product_name ?? '__legacy__'
        const key = `${log.source}:${log.receipt_number}:${productKey}`
    ...
    for (const [key, items] of groups) {
      ...
      const beforeVals = items.map(i => i.before_qty).filter((v): v is number => v != null)
      const afterVals  = items.map(i => i.after_qty).filter((v): v is number => v != null)
      rows.push({
        isGroup: true,
        key,
        source: first.source,
        ...
        totalChange: items.reduce((sum, i) => sum + i.change_qty, 0),
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 max-w-[200px]">
                          <span className="text-xs text-gray-500 truncate block" title={row.items.map(i => i.item_name).join(', ')}>

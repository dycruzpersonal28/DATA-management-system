...
// ── Cash Movement Edit Modal ───────────────────────────────────────────────────
function CashMovementEditModal({ movement, currencySymbol, onConfirm, onClose }: {
  movement: any
  currencySymbol: string
  onConfirm: (amount: number, note: string) => void
  onClose: () => void
}) {
  const [amount, setAmount] = useState(String(Number(movement._amount).toFixed(2)))
  const [note, setNote] = useState(movement.note || '')
  const isCashIn = movement._type === 'cash_in'
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{movement._staff}</p>
          </div>
          <button onClick={onClose} className="ml-auto p-1.5 rounded-lg text-gray-400 hover:bg-gray-100">
            <X className="w-4 h-4" />
          </button>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2.5 border border-gray-200 rounded-xl text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-blue-400"
                autoFocus
              />
            </div>
          </div>
        ...
        <div className="flex gap-2 mt-5">
          ...
          <button onClick={handleSave} className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition-all">
            Save Changes
          </button>
        </div>
      </div>
...
// ── Cash Movement Delete Confirm Modal ────────────────────────────────────────
function CashMovementDeleteModal({ movement, currencySymbol, onConfirm, onClose }: {
  movement: any
  currencySymbol: string
  onConfirm: () => void
  onClose: () => void
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900">
              Delete {isCashIn ? 'Cash In' : 'Cash Out'}
            </h3>
            <p className="text-xs text-gray-400">
              {currencySymbol}{Number(movement._amount).toFixed(2)} · {movement._staff}
            </p>
          </div>
          <button onClick={onClose} className="ml-auto p-1.5 rounded-lg text-gray-400 hover:bg-gray-100">
            <X className="w-4 h-4" />
        ...
        {movement.note && (
          <div className="mb-4 px-3 py-2 bg-gray-50 rounded-lg border border-gray-100">
            <p className="text-[10px] text-gray-400 uppercase font-semibold mb-0.5">Note</p>
            <p className="text-xs text-gray-600">{movement.note}</p>
          </div>
        )}

        <div className="bg-red-50 border border-red-200 rounded-xl p-3 mb-5">
          <p className="text-xs text-red-700 font-medium">
            ⚠️ This will permanently delete this entry. This cannot be undone.
          </p>
        </div>
...
export default function TransactionsPage() {
  ...
  const [pinAction, setPinAction] = useState<{ type: 'void' | 'edit' | 'reprint' | 'cash_edit' | 'cash_delete'; tx: any } | null>(null)
  ...
  const [cashEditModal, setCashEditModal]   = useState<{ tx: any; managerId: string; managerName: string } | null>(null)
  const [cashDeleteModal, setCashDeleteModal] = useState<{ tx: any; managerId: string; managerName: string } | null>(null)
  ...
  const filtered = allTransactions.filter(tx => {
    ...
    if (statusFilter !== 'all') {
      ...
      if (statusFilter === 'voided' && tx._type !== 'refund' && tx.status !== 'voided') return false
    }
  ...
  // ── Action handlers ──────────────────────────────────────────────────────────
  function requestAction(type: 'void' | 'edit' | 'reprint' | 'cash_edit' | 'cash_delete', tx: any) {
  ...
  function handlePinApproved(managerId: string, managerName: string) {
    ...
    if (type === 'void') {
      ...
      setEditReceipt({ tx, managerId, managerName })
    } else if (type === 'reprint') {
      handleReprint(tx)
    } else if (type === 'cash_edit') {
      setCashEditModal({ tx, managerId, managerName })
    } else if (type === 'cash_delete') {
      setCashDeleteModal({ tx, managerId, managerName })
    }
    setPinAction(null)
  }
  ...
  // ── Cash movement: edit ──────────────────────────────────────────────────────
  async function handleCashMovementEdit(movement: any, amount: number, note: string) {
    setCashEditModal(null)
    ...
    toast.success(`${movement._type === 'cash_in' ? 'Cash In' : 'Cash Out'} updated`)
    loadData()
  }
  ...
  async function handleCashMovementDelete(movement: any) {
    ...
    toast.success(`${movement._type === 'cash_in' ? 'Cash In' : 'Cash Out'} deleted`)
    loadData()
  }
    ...
    <div className="p-6 space-y-5">
      ...
      {cashEditModal && (
        <CashMovementEditModal
          movement={cashEditModal.tx}
          currencySymbol={currencySymbol}
          onConfirm={(amount, note) => handleCashMovementEdit(cashEditModal.tx, amount, note)}
          onClose={() => setCashEditModal(null)}
        />
      )}
      {cashDeleteModal && (
        <CashMovementDeleteModal
          movement={cashDeleteModal.tx}
          currencySymbol={currencySymbol}
          onConfirm={() => handleCashMovementDelete(cashDeleteModal.tx)}
          onClose={() => setCashDeleteModal(null)}
        />
      )}
                    ...
                    <React.Fragment key={`${tx._type}-${tx.id}`}>
                      ...
                      >
                        ...
                        <td className="px-3 py-2.5 whitespace-nowrap">
                          ...
                          {isCashIn && (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full font-medium bg-blue-100 text-blue-700">
                              <ArrowDownCircle className="w-2.5 h-2.5" /> Cash In
                            </span>
                          )}
                          {isCashOut && (
                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full font-medium bg-orange-100 text-orange-700">
                        ...
                        <td className="px-3 py-2.5 text-center">
                          {(isSale || isRefund) ? (
                            ...
                            <span className="inline-flex px-2 py-0.5 rounded-full font-medium bg-emerald-100 text-emerald-700">
                              completed
                            </span>
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex items-center justify-center gap-0.5">
                            ...
                            {/* Cash In / Cash Out: edit + delete */}
                            {(isCashIn || isCashOut) && (
                              <>
                                <button
                                  onClick={e => { e.stopPropagation(); requestAction('cash_edit', tx) }}
                                ...
                                <button
                                  onClick={e => { e.stopPropagation(); requestAction('cash_delete', tx) }}
